...
```
type IOrder = ICart & IContactData & IPaymentData
```
...
- addItem(item:IItem):void – добавляет id товара в корзину, пересчитывает total. Вызывает событие изменения корзины.
- removeItem(id:string):void – удаляет id товара из корзины, пересчитывает total. Вызывает событие изменения корзины.
...
- get total():number – геттер для итоговой стоимости товаров в корзине
...
– total: number, items: string[] – данные о составе заказа

###### Методы класса

- setData(data:Partial<IOrder>):void – обновляет хранимые в классе данные заказа и генерирует событие
- validatePayment(): boolean – валидация данных оплаты
...
– cartButton : HTMLButtonElement – кнопка добавления или удаления из корзины.
- events: IEvents – экземпляр класса EventEmitter для инициализации событий
...
Отображает счетчик товаров в корзине в шапке сайта. Устанавливает слушатель на нажатие по иконке и генерирует событие.
Конструктор принимает контейнер счетчика, а также экземпляр класса EventEmitter для возможности инициализации событий.

###### Поля:

- counter: HTMLElement – элемент, отображающий количество товаров.
- events: IEvents – экземпляр класса EventEmitter для инициализации событий
...
Реализует отображение форм в модальном окне. При сабмите инициирует событие передавая в него объект с данными из полей ввода формы. При изменении данных в полях ввода инициирует событие изменения данных. Предоставляет методы для отображения ошибок и управления активностью кнопки сабмита.
Конструктор принимает template и экземпляр EventEmitter.

###### Поля класса

- submitButton – кнопка отправки формы
- inputs: NodeListOf<HTMLInputElement> - инпут
  элементы формы
...
- `order-address:input` - изменение данных в поле ввода адреса формы
- `order-payment:changed` - изменение данных в поле выбора оплаты формы
- `contacts-email:input` - изменение данных в поле ввода email формы
- `contacts-phone:input` - изменение данных в поле ввода телефона формы
- `contacts:validation` - необходимость валидации формы контактов
- `order:validation` - необходимость валидации формы выбора способа оплаты и ввода адреса
- `contacts:submit` - сохранение данных в форме контактов
- `order:submit` - сохранение данных в форме выбора способа оплаты и адреса
